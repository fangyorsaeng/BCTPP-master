...
Page 50033 "Sales Billing"
{
    ...
    layout
    {
        area(content)
        {
            ...
            part(BillingLines; "Sales Billing Subform")
            {
                ApplicationArea = all;
            ...
            group("Function")
            {
                action("Get Cust. Ledger")
                {
                    ...
                    begin
                        Clear(getSalesInv);
                        SalesH.Reset();
                        SalesH.SetAutoCalcFields("Billing No.");
                        ...
                        SalesH.SetFilter("Billing No.", '=%1', '');
                        if SalesH.Find('-') then;
                        getSalesInv.setDocNo(Rec."No.", 'Sales');
                        getSalesInv.SetTableView(SalesH);
                        getSalesInv.RunModal();
                        //   GetCustLedger;
                    end;
                }
                action("Get Cust. Ledger2")
                {
                    Caption = 'Get Sale CR.Memo';
                    ApplicationArea = Basic;
                    Ellipsis = true;
                    Image = MakeOrder;
                    Promoted = true;
                    ...
                    begin
                        ...
                        SalesH.Reset();
                        SalesH.SetAutoCalcFields("Billing No.");
                        SalesH.SetRange("Document Type", SalesH."Document Type"::"Credit Memo");
                        SalesH.SetRange(Status, SalesH.Status::Released);
                        SalesH.SetRange("Sell-to Customer No.", Rec."Bill-to Customer No.");
                        SalesH.SetFilter("Billing No.", '=%1', '');
                        if SalesH.Find('-') then;
